           ...
           while(1){
               ...
               if(readVal == 'h' && toggle == 0){
                   ...
                   for(deg = 0; deg <= 180; deg += 2){
                       ...
                       if(calculatedIRVAL - calculatedPREVIOUSVAL < -6.66){
                                   //Start object
                                   objectStartDegree = deg;
                                   uart_sendChar('\n');
                                   uart_sendChar('\r');
                       ...
                       if(calculatedPREVIOUSVAL - calculatedIRVAL < -6.66){
                           //end object
                           objectEndDegree = deg;

                           if(objectEndDegree-objectStartDegree > 4 ){
                          //     if(((objectEndDegree+objectStartDegree)/2) > (objectList[objectCount--].degree + 3)){
                                   //object is valid
                                   uart_sendChar('\n');
                                   uart_sendChar('\r');
                ...
                //                   objectList[objectCount].distance = tempscan.sound_dist;
                  ...
                  if(readVal == 'h'){
                      ...
                      if(objectList[shortestObjectIndex].degree > 90){
                          //turn counter clockwise
                               turn_cclockwise(sensor_data, (objectList[shortestObjectIndex].degree - 90)*2.33); // debug: index out of bounds. ran, got index 20
                               move_forward_go_around(sensor_data, objectList[shortestObjectIndex].distance*4.77);


                      }
                      else{
                          //turn clockwise
                          turn_clockwise(sensor_data, (90-objectList[shortestObjectIndex].degree)*2.33);
                          move_forward_go_around(sensor_data, objectList[shortestObjectIndex].distance*4.77);
               ...
               if(toggle == 1){
                   ...
                   uart_sendChar(readVal);
                   if(readVal == 'w'){
                       move_forward_go_around(sensor_data, objectList[shortestObjectIndex].distance*5.75);
                   }
                   else if(readVal == 's'){
